...
const path& CFileList::getFileName(u32 index) const
{
	if (index >= Files.size())
...
const path& CFileList::getFullFileName(u32 index) const
{
	if (index >= Files.size())
		return emptyFileListEntry;
...
u32 CFileList::addItem(const path& fullPath, u32 offset, u32 size, bool isDirectory, u32 id)
{
...
s32 CFileList::findFile(const path& filename, bool isDirectory = false) const
{
	SFileListEntry entry;
