...
public:
	...
	//! constructor
	CNullDriver(IFileSystem* io, const dimension2d<u32>& screenSize);
	...
	//! creates a Texture
	virtual ITexture* addTexture(const dimension2d<u32>& size, const path& name, ECOLOR_FORMAT format = ECF_A8R8G8B8);
	...
	virtual void draw2DRectangle(SColor color, const rectangle<s32>& pos, const rectangle<s32>* clip = 0);
	...
	//! get screen size
	virtual const dimension2d<u32>& getScreenSize() const;
	...
	virtual void makeColorKeyTexture(ITexture* texture, SColor color, bool zeroTexels) const;

	//! Creates an 1bit alpha channel of the texture based of an color key position.
	virtual void makeColorKeyTexture(ITexture* texture, position2d<s32> colorKeyPixelPos, bool zeroTexels) const;

	//! Creates a normal map from a height map texture.
	//! \param amplitude: Constant value by which the height information is multiplied.
	virtual void makeNormalMapTexture(ITexture* texture, f32 amplitude=1.0f) const;
	...
	virtual IImage* createImage(ECOLOR_FORMAT format, const dimension2d<u32>& size);
...
public:
	...
	//! programs from files.
	virtual s32 addShaderMaterialFromFiles(IReadFile* vertexShaderProgram = 0,
	...
	//! Like IGPUProgrammingServices::addShaderMaterial(), but tries to load the
	//! programs from files.
	virtual s32 addShaderMaterialFromFiles(const path& vertexShaderProgramFileName,
	...
	virtual bool writeImageToFile(IImage* image, const path& filename, u32 param = 0);

	//! Writes the provided image to a file.
	virtual bool writeImageToFile(IImage* image, IWriteFile * file, u32 param = 0);
	...
	virtual stringc getVendorInfo() {return "Not available on this driver.";}
	...
	*/
	...
	//! deprecated method
	virtual ITexture* createRenderTargetTexture(const dimension2d<u32>& size,
			const c8* name=0);
...
protected:
	...
	struct SDummyTexture : public ITexture
	{
		...
		virtual void unlock(){}
		virtual const dimension2d<u32>& getOriginalSize() const { return size; }
		virtual const dimension2d<u32>& getSize() const { return size; }
	...
	array<SSurface> Textures;

	array<IImageLoader*> SurfaceLoader;
	array<IImageWriter*> SurfaceWriter;
	array<SLight> Lights;
	array<SMaterialRenderer> MaterialRenderers;

	IFileSystem* FileSystem;

	rectangle<s32> ViewPort;
