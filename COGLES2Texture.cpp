...
COGLES2Texture::COGLES2Texture( IImage* origImage, const path& name, COGLES2Driver* driver )
...
COGLES2Texture::COGLES2Texture( const path& name, COGLES2Driver* driver )
		: ITexture( name ), Driver( driver ), Image( 0 ),
		TextureName( 0 ), InternalFormat( GL_RGBA ), PixelFormat( GL_RGBA ),
		PixelType( GL_UNSIGNED_BYTE ),
		HasMipMaps( true ), IsRenderTarget( false ), AutomaticMipmapUpdate( false ),
...
void COGLES2Texture::getImageData( IImage* image )
{
	if ( !image )
	{
		Printer::log( "No image for OGLES2 texture.", ELL_ERROR );
		return;
	}
	...
	{
		Printer::log( "Invalid size of image for OGLES2 Texture.", ELL_ERROR );
		return;
	}

	const dimension2d<u32> nImageSize = ImageSize.getOptimalSize( !Driver->queryFeature( EVDF_TEXTURE_NPOT ) );
...
void COGLES2Texture::copyTexture( bool newTexture )
{
	if ( !Image )
	{
		Printer::log( "No image for OGLES2 texture to upload", ELL_ERROR );
		return;
	}
	...
	if ( Driver->testGLError() )
		Printer::log( "Could not bind Texture", ELL_ERROR );
...
#else
		HasMipMaps = false;
		Printer::log( "Did not create OGLES2 texture mip maps.", ELL_ERROR );
#endif
		{
	...
	if ( Driver->testGLError() )
		Printer::log( "Could not glTexImage2D", ELL_ERROR );
}
...
const dimension2d<u32>& COGLES2Texture::getOriginalSize() const
{
	return ImageSize;
}
...
const dimension2d<u32>& COGLES2Texture::getSize() const
{
	if ( Image )
...
COGLES2FBOTexture::COGLES2FBOTexture( const dimension2d<u32>& size,
									  const path& name,
		...
		: COGLES2Texture( name, driver ), DepthTexture( 0 ), ColorFrameBuffer( 0 )
{
	...
	switch ( col )
	{
		...
		case ECF_R5G6B5:
			...
			break;
		default:
			Printer::log( "color format not handled", ELL_WARNING );
			break;
	}
	PixelFormat = GL_RGBA;
	PixelType = GL_UNSIGNED_BYTE;
...
bool checkFBOStatus( COGLES2Driver* Driver )
{
	...
	switch ( status )
	{
		...
		case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_OES:
			Printer::log( "FBO has one or several incomplete image attachments", ELL_ERROR );
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_OES:
			Printer::log( "FBO missing an image attachment", ELL_ERROR );
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_OES:
			Printer::log( "FBO has one or several image attachments with different dimensions", ELL_ERROR );
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_OES:
			Printer::log( "FBO has one or several image attachments with different internal formats", ELL_ERROR );
			break;
...
// not part of all implementations
		...
		case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_OES:
			Printer::log( "FBO has invalid draw buffer", ELL_ERROR );
			break;
#endif
...
// not part of all implementations
		...
		case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_OES:
			Printer::log( "FBO has invalid read buffer", ELL_ERROR );
			break;
#endif
...
// not part of fbo_object anymore, but won't harm as it is just a return value
		...
		case GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_OES:
			Printer::log( "FBO has a duplicate image attachment", ELL_ERROR );
			break;
#endif

		case GL_FRAMEBUFFER_UNSUPPORTED_OES:
			Printer::log( "FBO format unsupported", ELL_ERROR );
			break;

		default:
			break;
	}
	Printer::log( "FBO error", ELL_ERROR );
	return false;
}
#endif
