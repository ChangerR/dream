...
bool CImageLoaderBMP::isALoadableFileFormat(IReadFile* file) const
{
	...
	headerID = Byteswap::byteswap(headerID);
#endif
...
IImage* CImageLoaderBMP::loadImage(IReadFile* file) const
{
...
#ifdef __BIG_ENDIAN__
	header.Id = Byteswap::byteswap(header.Id);
	header.FileSize = Byteswap::byteswap(header.FileSize);
	header.BitmapDataOffset = Byteswap::byteswap(header.BitmapDataOffset);
	header.BitmapHeaderSize = Byteswap::byteswap(header.BitmapHeaderSize);
	header.Width = Byteswap::byteswap(header.Width);
	header.Height = Byteswap::byteswap(header.Height);
	header.Planes = Byteswap::byteswap(header.Planes);
	header.BPP = Byteswap::byteswap(header.BPP);
	header.Compression = Byteswap::byteswap(header.Compression);
	header.BitmapDataSize = Byteswap::byteswap(header.BitmapDataSize);
	header.PixelPerMeterX = Byteswap::byteswap(header.PixelPerMeterX);
	header.PixelPerMeterY = Byteswap::byteswap(header.PixelPerMeterY);
	header.Colors = Byteswap::byteswap(header.Colors);
	header.ImportantColors = Byteswap::byteswap(header.ImportantColors);
#endif
	...
	{
		Printer::log("Compression mode not supported.", ELL_ERROR);
		return 0;
	}
	...
	if (paletteSize)
	{
		...
		for (s32 i=0; i<paletteSize; ++i)
			paletteData[i] = Byteswap::byteswap(paletteData[i]);
#endif
	}
