...
static IIEventListener::ListenerID __getListenerID(Event* event)
{
    IIEventListener::ListenerID ret;
    switch (event->getType())
    {
        ...
        case Event::Type::TOUCH:
            ...
            Printer::log("Don't call this method if the event is for touch.");
            break;
        case Event::Type::GAME_CONTROLLER:
            ret = "";
            break;
        default:
		    Printer::log("Invalid type!");
		    break;
    }
...
CCEventDispatcher::EventListenerVector::EventListenerVector() :
...
size_t CCEventDispatcher::EventListenerVector::size() const
{
    size_t ret = 0;
...
bool CCEventDispatcher::EventListenerVector::empty() const
{
    return (_sceneGraphListeners == nullptr || _sceneGraphListeners->empty())
        && (_fixedListeners == nullptr || _fixedListeners->empty());
}

void CCEventDispatcher::EventListenerVector::push_back(IEventListener* listener)
{
    if (listener->getFixedPriority() == 0)
    {
        if (_sceneGraphListeners == nullptr)
        {
            _sceneGraphListeners = new std::vector<IEventListener*>();
    ...
    else
    {
        ...
        {
            _fixedListeners = new std::vector<IEventListener*>();
...
void CCEventDispatcher::EventListenerVector::clearSceneGraphListeners()
{
    if (_sceneGraphListeners)
    {
...
void CCEventDispatcher::EventListenerVector::clearFixedListeners()
{
    if (_fixedListeners)
    {
...
void CCEventDispatcher::EventListenerVector::clear()
{
...
void CEventDispatcher::visitTarget(Node* node, bool isRootNode)
{    
    int i = 0;
...
void CEventDispatcher::pauseEventListenersForTarget(Node* target, bool recursive/* = false */)
{
...
void CEventDispatcher::resumeEventListenersForTarget(Node* target, bool recursive/* = false */)
{
    auto listenerIter = _nodeListenersMap.find(target);
    if (listenerIter != _nodeListenersMap.end())
    {
...
void CEventDispatcher::removeEventListenersForTarget(Node* target, bool recursive/* = false */)
{
...
void CEventDispatcher::associateNodeAndEventListener(Node* node, IEventListener* listener)
{
    std::vector<IEventListener*>* listeners = nullptr;
    ...
    {
        listeners = new std::vector<IEventListener*>();
...
void CEventDispatcher::dissociateNodeAndEventListener(Node* node, IEventListener* listener)
{
    std::vector<IEventListener*>* listeners = nullptr;
    auto found = _nodeListenersMap.find(node);
    if (found != _nodeListenersMap.end())
...
void CEventDispatcher::addEventListener(IEventListener* listener)
{
    if (_inDispatch == 0)
    {
...
void CEventDispatcher::forceAddEventListener(IEventListener* listener)
{
    EventListenerVector* listeners = nullptr;
    IEventListener::ListenerID listenerID = listener->getListenerID();
...
void CEventDispatcher::addEventListenerWithSceneGraphPriority(IEventListener* listener, Node* node)
{
...
void CEventDispatcher::debugCheckNodeHasNoEventListenersOnDestruction(Node* node)
{
...
void CEventDispatcher::addEventListenerWithFixedPriority(IEventListener* listener, int fixedPriority)
{
...
EventListenerCustom* CEventDispatcher::addCustomEventListener(const stringc &eventName, const std::function<void(EventCustom*)>& callback)
{
...
void CEventDispatcher::removeEventListener(IEventListener* listener)
{
    if (listener == nullptr)
        return;
    ...
    auto removeListenerInVector = [&](std::vector<IEventListener*>* listeners){
...
void CEventDispatcher::setPriority(IEventListener* listener, int fixedPriority)
{
    if (listener == nullptr)
        return;
...
void CEventDispatcher::dispatchEventToListeners(EventListenerVector* listeners, const std::function<bool(IEventListener*)>& onEvent)
{
...
void CEventDispatcher::dispatchEvent(Event* event)
{
    if (!_isEnabled)
        return;
    ...
    if (iter != _listenerMap.end())
    {
        ...
        auto onEvent = [&event](IEventListener* listener) -> bool{
...
void CEventDispatcher::dispatchCustomEvent(const stringc &eventName, void *optionalUserData)
{
...
void CEventDispatcher::dispatchTouchEvent(EventTouch* event)
{
    ...
    if (oneByOneListeners)
    {
        ...
        for (; touchesIter != originalTouches.end(); ++touchesIter)
        {
            ...
            auto onTouchEvent = [&](IEventListener* l) -> bool { // Return true to break
    ...
    if (allAtOnceListeners && mutableTouches.size() > 0)
    {
        ...
        auto onTouchesEvent = [&](IEventListener* l) -> bool{
...
void CEventDispatcher::updateListeners(Event* event)
{
    CCASSERT(_inDispatch > 0, "If program goes here, there should be event in dispatch.");
    
    auto onUpdateListeners = [this](const IEventListener::ListenerID& listenerID)
    {
...
void CEventDispatcher::updateDirtyFlagForSceneGraph()
{
...
void CEventDispatcher::sortEventListeners(const IEventListener::ListenerID& listenerID)
{
...
void CEventDispatcher::sortEventListenersOfSceneGraphPriority(const IEventListener::ListenerID& listenerID, Node* rootNode)
{
    ...
    std::sort(sceneGraphListeners->begin(), sceneGraphListeners->end(), [this](const IEventListener* l1, const IEventListener* l2) {
...
void CEventDispatcher::sortEventListenersOfFixedPriority(const IEventListener::ListenerID& listenerID)
{
    auto listeners = getListeners(listenerID);
    ...
    // After sort: priority < 0, > 0
    std::sort(fixedListeners->begin(), fixedListeners->end(), [](const IEventListener* l1, const IEventListener* l2) {
...
CEventDispatcher::EventListenerVector* CEventDispatcher::getListeners(const IEventListener::ListenerID& listenerID)
{
    auto iter = _listenerMap.find(listenerID);
    if (iter != _listenerMap.end())
    {
...
void CEventDispatcher::removeEventListenersForListenerID(const IEventListener::ListenerID& listenerID)
{
    ...
    if (listenerItemIter != _listenerMap.end())
    {
        ...
        auto removeAllListenersInVector = [&](std::vector<IEventListener*>* listenerVector){
...
void CEventDispatcher::removeEventListenersForType(IEventListener::Type listenerType)
{
    if (listenerType == IEventListener::Type::TOUCH_ONE_BY_ONE)
    {
        removeEventListenersForListenerID(EventListenerTouchOneByOne::LISTENER_ID);
    }
    else if (listenerType == IEventListener::Type::TOUCH_ALL_AT_ONCE)
    {
        removeEventListenersForListenerID(EventListenerTouchAllAtOnce::LISTENER_ID);
    }
    else if (listenerType == IEventListener::Type::MOUSE)
    {
        removeEventListenersForListenerID(EventListenerMouse::LISTENER_ID);
    }
    else if (listenerType == IEventListener::Type::ACCELERATION)
    {
        removeEventListenersForListenerID(EventListenerAcceleration::LISTENER_ID);
    }
    else if (listenerType == IEventListener::Type::KEYBOARD)
    {
...
void CEventDispatcher::removeCustomEventListeners(const stringc& customEventName)
{
    removeEventListenersForListenerID(customEventName);
}

void CEventDispatcher::removeAllEventListeners()
{
    bool cleanMap = true;
    std::vector<IEventListener::ListenerID> types(_listenerMap.size());
...
void CEventDispatcher::setEnabled(bool isEnabled)
{
    _isEnabled = isEnabled;
}

bool CEventDispatcher::isEnabled() const
{
    return _isEnabled;
}

void CEventDispatcher::setDirtyForNode(Node* node)
{
...
void CEventDispatcher::setDirty(const IEventListener::ListenerID& listenerID, DirtyFlag flag)
{    
