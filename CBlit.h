...
/*
*/
inline void GetClip(AbsRectangle &clipping, IImage * t)
{
	clipping.x0 = 0;
	clipping.y0 = 0;
...
/*
*/
static void RenderLine32_Decal(IImage *t,
...
/*
*/
static void RenderLine16_Decal(IImage *t,
				const position2d<s32> &p0,
				const position2d<s32> &p1,
				u32 argb )
{
...
/*
*/
static void RenderLine16_Blend(IImage *t,
				const position2d<s32> &p0,
				const position2d<s32> &p1,
				u16 argb,
				u16 alpha)
					 ...
					 const IImage * tex, s32 passnative )
{
	...
	if ( clip )
	{
		out.x0 = s32_clamp ( clip->UpperLeftCorner.X, 0, w );
		out.x1 = s32_clamp ( clip->LowerRightCorner.X, out.x0, w );
		out.y0 = s32_clamp ( clip->UpperLeftCorner.Y, 0, h );
		out.y1 = s32_clamp ( clip->LowerRightCorner.Y, out.y0, h );
	}
	else
	{
		out.x0 = 0;
...
static void drawLine(IImage* img, const position2d<s32>& from,
					 const position2d<s32>& to, const SColor &color)
{
	AbsRectangle clip;
	GetClip(clip, img);
