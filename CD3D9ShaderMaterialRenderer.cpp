	...
	if (material.MaterialType != lastMaterial.MaterialType || resetAllRenderstates)
	{
		if (VertexShader)
		{
			...
			if (FAILED(pID3DDevice->SetVertexShader(VertexShader)))
				Printer::log("Could not set vertex shader.", ELL_WARNING);
		}
		...
		if (PixelShader)
		{
			if (FAILED(pID3DDevice->SetPixelShader(PixelShader)))
				Printer::log("Could not set pixel shader.", ELL_WARNING);
		}
...
bool CD3D9ShaderMaterialRenderer::createPixelShader(const c8* pxsh)
{
	...
	if (errors)
	{
		...
		Printer::log("Pixel shader compilation failed:", ELL_ERROR);
	...
	{
		Printer::log("Could not create pixel shader.", ELL_ERROR);
		code->Release();
		return false;
	}
...
bool CD3D9ShaderMaterialRenderer::createVertexShader(const char* vtxsh)
{
	...
	{
		// print out compilation errors.
		Printer::log("Vertex shader compilation failed:", ELL_ERROR);
		Printer::log((c8*)errors->GetBufferPointer(), ELL_ERROR);
	...
	{
		Printer::log("Could not create vertex shader.", ELL_ERROR);
		if (code)
			code->Release();
		return false;
	}
		...
		LPD3DXBUFFER* ppErrorMsgs)
{
	...
	#else
	{
		...
		if (!pFn && !LoadFailed)
		{
			...
			{
				LoadFailed = true;
				Printer::log("Could not load shader function D3DXAssembleShader from dll, shaders disabled",
			...
			LPD3DXBUFFER* ppShader, LPD3DXBUFFER* ppErrorMsgs)
{
	...
	#else
	{
		...
		if (!pFn && !LoadFailed)
		{
			...
			{
				LoadFailed = true;
				Printer::log("Could not load shader function D3DXAssembleShaderFromFileA from dll, shaders disabled",
					strDllName.c_str(), ELL_ERROR);
			}
		}
