...
bool CD3D9Driver::initDriver(HWND hwnd, bool pureSoftware)
{
	if (!pID3D)
	{
		...
		if (!d3dCreate)
		{
			Printer::log("Error, could not get proc adress of Direct3DCreate9.", ELL_ERROR);
			return false;
		}
	...
	#ifndef _DREAM_D3D_NO_SHADER_DEBUGGING
	devtype = D3DDEVTYPE_REF;
	#elif defined(_DREAM_USE_NVIDIA_PERFHUD_)
	...
	if (pureSoftware)
	{
		if (FAILED(pID3D->CreateDevice(Params.DisplayAdapter, D3DDEVTYPE_REF, hwnd,
			...
			Printer::log("Was not able to create Direct3D9 software device.", ELL_ERROR);
	}
	else
	{
	...
	Printer::log("Currently available Video Memory (kB)", stringc(pID3DDevice->GetAvailableTextureMem()/1024).c_str());
	...
	{
		Printer::log("Was not able to get main depth buffer.", ELL_ERROR);
		return false;
	}
...
bool CD3D9Driver::endScene()
{
	...
	{
		DeviceLost = true;
		Printer::log("Present failed", "DIRECT3D9 device lost.", ELL_WARNING);
	}
#ifdef D3DERR_DEVICEREMOVED
	else if (hr == D3DERR_DEVICEREMOVED)
	{
		Printer::log("Present failed", "Device removed.", ELL_WARNING);
	}
#endif
	else if (hr == D3DERR_INVALIDCALL)
	{
		Printer::log("Present failed", "Invalid Call", ELL_WARNING);
	}
	else
...
ITexture* CD3D9Driver::createDeviceDependentTexture(IImage* surface,const path& name, void* mipmapData)
{
		...
		bool clearBackBuffer, bool clearZBuffer, SColor color)
{
	...
	if (tex == 0)
	{
		if (PrevRenderTarget)
		{
			...
			{
				Printer::log("Error: Could not set main depth buffer.", ELL_ERROR);
			}
	...
	else
	{
		...
		{
			Printer::log("Error: Could not set new depth buffer.", ELL_ERROR);
		}
	}
	Transformation3DChanged=true;
...
bool CD3D9Driver::setRenderTarget(const array<IRenderTarget>& targets,
				bool clearBackBuffer, bool clearZBuffer, SColor color)
{
	if (targets.size()==0)
		return setRenderTarget(0, clearBackBuffer, clearZBuffer, color);

	u32 maxMultipleRTTs = min_(MaxMRTs, targets.size());
	...
	for (u32 i = 0; i < maxMultipleRTTs; ++i)
	{
		...
		{
			maxMultipleRTTs = i;
			Printer::log("Missing texture for MRT.", ELL_WARNING);
			break;
		}
		...
		{
			maxMultipleRTTs = i;
			Printer::log("Tried to set a texture not owned by this driver.", ELL_WARNING);
			break;
		}
		...
		{
			maxMultipleRTTs = i;
			Printer::log("Tried to set a non render target texture as render target.", ELL_WARNING);
			break;
		}
		...
		{
			maxMultipleRTTs = i;
			Printer::log("Render target texture has wrong size.", ELL_WARNING);
			break;
		}
	}
	if (maxMultipleRTTs==0)
	{
		Printer::log("Fatal Error: No valid MRT found.", ELL_ERROR);
		return false;
	}
...
//! sets a viewport
void CD3D9Driver::setViewPort(const rectangle<s32>& area)
{
	...
	if (vp.getHeight()>0 && vp.getWidth()>0)
	{
		...
		if (FAILED(hr))
			Printer::log("Failed setting the viewport.", ELL_WARNING);
		else
			ViewPort = vp;
	}
}
...
const rectangle<s32>& CD3D9Driver::getViewPort() const
{
	return ViewPort;
}
...
void CD3D9Driver::draw2DRectangle(const rectangle<s32>& position,
...
//! Draws a 2d line.
void CD3D9Driver::draw2DLine(const position2d<s32>& start,
...
void CD3D9Driver::setVertexShader(E_VERTEX_TYPE newType)
{
	if (newType != LastVertexType)
	{
		...
		if (FAILED(hr))
		{
			Printer::log("Could not set vertex Shader.", ELL_ERROR);
			return;
		}
	}
}
...
//		if (!texture)
//			m.setTranslation(vector3df(0.5f,0.5f,0));
...
void CD3D9Driver::drawStencilShadowVolume(const array<vector3df>& triangles, bool zfail, u32 debugDataVisible)
{
	if (!Params.Stencilbuffer)
		return;
	...
	if (!zfail)
	{
		...
		pID3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, count / 3, triangles.const_pointer(), sizeof(vector3df));
...
//! Draws a 3d line.
void CD3D9Driver::draw3DLine(const vector3df& start,
...
bool CD3D9Driver::reset()
{
	...
	if (FAILED(hr))
	{
		...
		{
			Printer::log("Resetting failed due to device removed.", ELL_WARNING);
		}
#endif
		else if (hr == D3DERR_DRIVERINTERNALERROR)
		{
			Printer::log("Resetting failed due to internal error.", ELL_WARNING);
		}
		else if (hr == D3DERR_OUTOFVIDEOMEMORY)
		{
			Printer::log("Resetting failed due to out of memory.", ELL_WARNING);
		}
		else if (hr == D3DERR_DEVICENOTRESET)
		{
			Printer::log("Resetting failed due to not reset.", ELL_WARNING);
		}
		else if (hr == D3DERR_INVALIDCALL)
		{
			Printer::log("Resetting failed due to invalid call", "You need to release some more surfaces.", ELL_WARNING);
		}
		else
		{
			Printer::log("Resetting failed due to unknown reason.", stringc((int)hr).c_str(), ELL_WARNING);
		}
		return false;
	}
...
void CD3D9Driver::OnResize(const dimension2d<u32>& size)
{
	if (!pID3DDevice)
		return;
...
ITexture* CD3D9Driver::addRenderTargetTexture(const dimension2d<u32>& size,
											  const path& name,
...
void CD3D9Driver::clearZBuffer()
{
	...
	if (FAILED(hr))
		Printer::log("CD3D9Driver clearZBuffer() failed.", ELL_WARNING);
}
...
const dimension2d<u32>& CD3D9Driver::getCurrentRenderTargetSize() const
{
	...
	if (!depth)
	{
		...
		else
		{
			if (hr == D3DERR_OUTOFVIDEOMEMORY)
				Printer::log("Could not create DepthBuffer","out of video memory",ELL_ERROR);
			else if( hr == E_OUTOFMEMORY )
				Printer::log("Could not create DepthBuffer","out of memory",ELL_ERROR);
			else
			{
				char buffer[128];
				sprintf(buffer,"Could not create DepthBuffer of %ix%i",optSize.Width,optSize.Height);
				Printer::log(buffer,ELL_ERROR);
			}
