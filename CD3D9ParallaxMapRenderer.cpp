...
bool CD3D9ParallaxMapRenderer::OnRender(IMaterialRendererServices* service, E_VERTEX_TYPE vtxtype)
{
	...
	{
		Printer::log("Error: Parallax map renderer only supports vertices of type EVT_TANGENTS", ELL_ERROR);
		return false;
	}
...
void CD3D9ParallaxMapRenderer::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{
	...
	services->setVertexShaderConstant(driver->getTransform(ETS_WORLD).getTransposed().pointer(), 0, 4);
