...
public:
	...
	virtual dimension2d<u32> getVideoModeResolution(s32 modeNumber) const;

	//! Returns the screen size of an optimal video mode in pixels.
	virtual dimension2d<u32> getVideoModeResolution(const dimension2d<u32>& minSize, const dimension2d<u32>& maxSize) const;
	...
	virtual const dimension2d<u32>& getDesktopResolution() const;

	//! Returns the pixel depth of a video mode in bits.
	virtual s32 getDesktopDepth() const;

	//! adds a new mode to the list
	void addMode(const dimension2d<u32>& size, s32 depth);

	void setDesktop(s32 desktopDepth, const dimension2d<u32>& desktopSize);
