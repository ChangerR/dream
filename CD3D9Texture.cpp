...
CD3D9Texture::CD3D9Texture(CD3D9Driver* driver, const dimension2d<u32>& size,
...
CD3D9Texture::CD3D9Texture(IImage* image, CD3D9Driver* driver,
			   u32 flags, const path& name, void* mipmapData)
: ITexture(name), Texture(0), RTTSurface(0), Driver(driver), DepthSurface(0),
...
void CD3D9Texture::createRenderTarget(const ECOLOR_FORMAT format)
{
	...
	if(!Driver->queryFeature(EVDF_TEXTURE_NPOT))
	{
		if (TextureSize != ImageSize)
			Printer::log("RenderTarget size has to be a power of two", ELL_INFORMATION);
	}
	...
	if (FAILED(hr))
	{
		if (D3DERR_INVALIDCALL == hr)
			Printer::log("Could not create render target texture", "Invalid Call");
		else
		if (D3DERR_OUTOFVIDEOMEMORY == hr)
			Printer::log("Could not create render target texture", "Out of Video Memory");
		else
		if (E_OUTOFMEMORY == hr)
			Printer::log("Could not create render target texture", "Out of Memory");
		else
...
bool CD3D9Texture::createTexture(u32 flags, IImage * image)
{
	...
	dimension2d<u32> optSize = ImageSize.getOptimalSize(!Driver->queryFeature(EVDF_TEXTURE_NPOT), !Driver->queryFeature(EVDF_TEXTURE_NSQUARE), true, Driver->Caps.MaxTextureWidth);
...
bool CD3D9Texture::copyTexture(IImage * image)
{
	if (Texture && image)
	{
		...
		if (FAILED(hr))
		{
			Printer::log("Texture data not copied", "Could not LockRect D3D9 Texture.", ELL_ERROR);
			return false;
		}
		...
		if (FAILED(hr))
		{
			Printer::log("Texture data not copied", "Could not UnlockRect D3D9 Texture.", ELL_ERROR);
			return false;
		}
	}
...
void* CD3D9Texture::lock(E_TEXTURE_LOCK_MODE mode, u32 mipmapLevel)
{
	...
	else
	{
		if (!RTTSurface)
		{
			...
			if (FAILED(hr))
			{
				Printer::log("Could not lock DIRECT3D9 Texture", "Offscreen surface creation failed.", ELL_ERROR);
				return 0;
			}
		}
		...
		if (FAILED(hr))
		{
			Printer::log("Could not lock DIRECT3D9 Texture", "Could not get surface.", ELL_ERROR);
			return 0;
		}
		...
		if(FAILED(hr))
		{
			Printer::log("Could not lock DIRECT3D9 Texture", "Data copy failed.", ELL_ERROR);
			return 0;
		}
		hr = RTTSurface->LockRect(&rect, 0, (mode==ETLM_READ_ONLY)?D3DLOCK_READONLY:0);
		if(FAILED(hr))
		{
			Printer::log("Could not lock DIRECT3D9 Texture", "LockRect failed.", ELL_ERROR);
			return 0;
		}
	}
	return rect.pBits;
...
const dimension2d<u32>& CD3D9Texture::getOriginalSize() const
{
	return ImageSize;
}
...
const dimension2d<u32>& CD3D9Texture::getSize() const
{
	return TextureSize;
}
...
void CD3D9Texture::regenerateMipMapLevels(void* mipmapData)
{
	if (mipmapData)
	{
		...
		do
		{
			...
			{
				Printer::log("Could not get mipmap level", ELL_WARNING);
				return;
			}
			...
			{
				mipSurface->Release();
				Printer::log("Could not lock texture", ELL_WARNING);
				return;
			}
