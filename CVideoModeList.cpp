...
{
	Desktop.depth = 0;
	Desktop.size = dimension2d<u32>(0,0);
}


void CVideoModeList::setDesktop(s32 desktopDepth, const dimension2d<u32>& desktopSize)
{
...
dimension2d<u32> CVideoModeList::getVideoModeResolution(s32 modeNumber) const
{
	if (modeNumber < 0 || modeNumber > (s32)VideoModes.size())
		return dimension2d<u32>(0,0);

	return VideoModes[modeNumber].size;
}


dimension2d<u32> CVideoModeList::getVideoModeResolution(
		const dimension2d<u32>& minSize,
		const dimension2d<u32>& maxSize) const
{
...
const dimension2d<u32>& CVideoModeList::getDesktopResolution() const
{
	return Desktop.size;
}
...
void CVideoModeList::addMode(const dimension2d<u32>& size, s32 depth)
{
	SVideoMode m;
	m.depth = depth;
	m.size = size;
