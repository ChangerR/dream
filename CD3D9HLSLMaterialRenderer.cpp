	...
	VSConstantsTable(0), PSConstantsTable(0)
{
	...
	{
		Printer::log("Invalid HLSL vertex shader compilation target", ELL_ERROR);
		return;
	}
			...
			const char* shaderTargetName)
{
...
#ifdef _DREAM_D3D_NO_SHADER_DEBUGGING
	...
	if (FAILED(h))
	{
		Printer::log("HLSL vertex shader compilation failed:", ELL_ERROR);
		if (errors)
		{
			Printer::log((c8*)errors->GetBufferPointer(), ELL_ERROR);
			errors->Release();
			if (buffer)
				buffer->Release();
		}
	...
	if (buffer)
	{
		if (FAILED(pID3DDevice->CreateVertexShader((DWORD*)buffer->GetBufferPointer(),
			&VertexShader)))
		{
			Printer::log("Could not create hlsl vertex shader.", ELL_ERROR);
			buffer->Release();
			return false;
		}
		...
		const char* shaderTargetName)
{
...
#endif
#if defined(_DREAM_D3D_USE_LEGACY_HLSL_COMPILER) && defined(D3DXSHADER_USE_LEGACY_D3DX9_31_DLL)
#ifdef D3DXSHADER_ENABLE_BACKWARDS_COMPATIBILITY
	else
#endif
	...
	if (FAILED(h))
	{
		Printer::log("HLSL pixel shader compilation failed:", ELL_ERROR);
		if (errors)
		{
			Printer::log((c8*)errors->GetBufferPointer(), ELL_ERROR);
			errors->Release();
	...
	if (buffer)
	{
		if (FAILED(pID3DDevice->CreatePixelShader((DWORD*)buffer->GetBufferPointer(),
			&PixelShader)))
		{
			Printer::log("Could not create hlsl pixel shader.", ELL_ERROR);
			buffer->Release();
			return false;
		}
					...
					const bool* bools, int count)
{
	...
	if(Description.RegisterSet != D3DXRS_SAMPLER)
	{
		...
		if (FAILED(hr))
		{
			Printer::log("Error setting bool array for HLSL variable", ELL_WARNING);
			return false;
		}
	}
					...
					const s32* ints, int count)
{
	...
	if(Description.RegisterSet != D3DXRS_SAMPLER)
	{
		...
		if (FAILED(hr))
		{
			Printer::log("Error setting int array for HLSL variable", ELL_WARNING);
			return false;
		}
	}
