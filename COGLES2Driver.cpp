...
COGLES2Driver::COGLES2Driver( const SDreamCreationParameters& params,
                              const SExposedVideoData& data, IFileSystem* io
                            )
...
#endif
#if defined(_DREAM_COMPILE_WITH_WINDOWS_DEVICE_)
	  , HDc( 0 )
#endif
	  ...
	  , DestFactor( EBF_ZERO ) 
{
	...
	if ( EglDisplay == EGL_NO_DISPLAY ) {
		Printer::log( "Getting OpenGL-ES2 display." );
		EglDisplay = eglGetDisplay(( NativeDisplayType ) EGL_DEFAULT_DISPLAY );
	}
	if ( EglDisplay == EGL_NO_DISPLAY ) {
		Printer::log( "Could not get OpenGL-ES2 display." );
	}

	EGLint majorVersion, minorVersion;
	if ( !eglInitialize( EglDisplay, &majorVersion, &minorVersion ) ) {
		Printer::log( "Could not initialize OpenGL-ES2 display." );
	} else {
		char text[64];
	...
	if ( !eglChooseConfig( EglDisplay, attribs, &config, 1, &num_configs ) ) {
		Printer::log( "Could not get config for OpenGL-ES2 display." );
	} else {
		char log[64];
		snprintf( log, 64, "Got %d configs.\n", num_configs );
		Printer::log( log );
	}
	Printer::log( " Creating EglSurface with nativeWindow..." );
	...
	if ( EGL_NO_SURFACE == EglSurface ) {
		...
		Printer::log( "Creating EglSurface without nativeWindows..." );
	} else
		Printer::log( "SUCCESS\n" );
	if ( EGL_NO_SURFACE == EglSurface ) {
		Printer::log( "FAILED\n" );
		Printer::log( "Could not create surface for OpenGL-ES2 display." );
	} else
		Printer::log( "SUCCESS\n" );
...
#endif
	Printer::log( "Creating EglContext..." );
	EglContext = eglCreateContext( EglDisplay, config, EGL_NO_CONTEXT, contextAttrib );
	if ( testEGLError() ) {
		Printer::log( "FAILED\n" );
		Printer::log( "Could not create Context for OpenGL-ES2 display." );
	}

	eglMakeCurrent( EglDisplay, EglSurface, EglSurface, EglContext );
	if ( testEGLError() ) {
		Printer::log( "Could not make Context current for OpenGL-ES2 display." );
	}
...
#endif
#ifdef _DREAM_COMPILE_WITH_ANDROID_DEVICE_
	Printer::log( "OpenGL-ES2 initialized" );
	__android_log_print(ANDROID_LOG_INFO, "dream", "OGLES2 initialized");
	genericDriverInit( params.WindowSize, params.Stencilbuffer );
#endif
}
...
bool COGLES2Driver::genericDriverInit( const dimension2d<u32>& screenSize, bool stencilBuffer ) {
	Name = glGetString( GL_VERSION );
	printVersion();
#ifndef _DREAM_COMPILE_WITH_ANDROID_DEVICE_
	Printer::log( eglQueryString( EglDisplay, EGL_CLIENT_APIS ) );
#endif
	// print renderer information
	vendorName = glGetString( GL_VENDOR );
	Printer::log( vendorName.c_str(), ELL_INFORMATION );
	...
	if ( FeatureAvailable[DREAM_separate_specular_color] )
...
bool COGLES2Driver::endScene() {
	...
	if ( EGL_SUCCESS != g ) {
		if ( EGL_CONTEXT_LOST == g ) {
			...
			Printer::log( "Context lost, please restart your app." );
		} else
			Printer::log( "Could not swap buffers for OpenGL-ES2 driver." );
		return false;
	}
#endif
	return true;
...
const matrix4& COGLES2Driver::getTransform( E_TRANSFORMATION_STATE state ) const {
	return Matrices[state];
}


//! sets transformation
void COGLES2Driver::setTransform( E_TRANSFORMATION_STATE state, const matrix4& mat ) {
	Matrices[state] = mat;
...
void COGLES2Driver::drawVertexPrimitiveList( const void* vertices, u32 vertexCount,
	...
	if ( static_cast<u32>( Material.MaterialType ) < MaterialRenderers.size() )
		MaterialRenderers[Material.MaterialType].Renderer->PostRender( this, EVT_STANDARD );
}


void COGLES2Driver::drawVertexPrimitiveList2d3d( const void* vertices, u32 vertexCount,
        const void* indexList, u32 primitiveCount,
        E_VERTEX_TYPE vType, E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType, bool threed ) {
	...
	if ( NoHighLevelShader ) {
		...
		else if ( FeatureAvailable[DREAM_OES_point_size_array] && ( Material.Thickness == 0.0f ) )
			glEnableClientState( GL_POINT_SIZE_ARRAY_OES );
#endif
		if ( threed && ( pType != EPT_POINTS ) && ( pType != EPT_POINT_SPRITES ) ) {
...
#ifdef GL_OES_point_size_array
				if (( pType == EPT_POINTS ) || ( pType == EPT_POINT_SPRITES ) ) {
					if ( FeatureAvailable[DREAM_OES_point_size_array] && ( Material.Thickness == 0.0f ) )
...
#define GL_UNSIGNED_INT                   0x1405
#endif
		if ( FeatureAvailable[DREAM_OES_element_index_uint] )
	...
	case EPT_POINT_SPRITES: {
		...
		if ( pType == EPT_POINT_SPRITES && FeatureAvailable[DREAM_OES_point_sprite] )
...
void COGLES2Driver::draw2DImage( const ITexture* texture,
	...
	position2d<s32> targetPos( pos );
	position2d<s32> sourcePos( sourceRect.UpperLeftCorner );
	...
	const dimension2d<u32>& renderTargetSize = getCurrentRenderTargetSize();
	...
	drawVertexPrimitiveList2d3d( vertices, 4, indices, 2, EVT_STANDARD, EPT_TRIANGLE_FAN, EIT_16BIT, false );
}

void COGLES2Driver::draw2DImageBatch( const ITexture* texture,
	...
	for ( u32 i = 0; i < drawCount; i++ ) {
		position2d<s32> targetPos = positions[i];
		position2d<s32> sourcePos = sourceRects[i].UpperLeftCorner;
...
void COGLES2Driver::draw2DImage( const ITexture* texture, const rectangle<s32>& destRect,
...
void COGLES2Driver::draw2DImageBatch( const ITexture* texture,
                                      const position2d<s32>& pos,
                                      const array<rectangle<s32> >& sourceRects,
                                      const array<s32>& indices, s32 kerningWidth,
                                      const rectangle<s32>* clipRect, SColor color,
                                      bool useAlphaChannelOfTexture ) {
	if ( !texture )
		return;
	...
	for ( u32 i = 0; i < indices.size(); ++i ) {
		...
		const rectangle<s32> poss( targetPos, sourceRects[currentIndex].getSize() );
	...
	drawVertexPrimitiveList2d3d( vertices.pointer(), 4, quadIndices.pointer(), 2*indices.size(), EVT_STANDARD, EPT_TRIANGLES, EIT_16BIT, false );
	if ( clipRect )
		glDisable( GL_SCISSOR_TEST );
	testGLError();
}
...
//! draw a 2d rectangle
void COGLES2Driver::draw2DRectangle( SColor color, const rectangle<s32>& position,
                                     const rectangle<s32>* clip ) {
	disableTextures();
	setRenderStates2DMode( color.getAlpha() < 255, false, false );

	rectangle<s32> pos = position;
...
void COGLES2Driver::draw2DRectangle( const rectangle<s32>& position,
...
//! Draws a 2d line.
void COGLES2Driver::draw2DLine( const position2d<s32>& start,
...
bool COGLES2Driver::setTexture( u32 stage, const ITexture* texture ) {
	...
	if ( !texture ) {
		...
		if ( texture->getDriverType() != EDT_OGLES2 ) {
			Printer::log( "Fatal Error: Tried to set a texture not owned by this driver.", ELL_ERROR );
			return false;
		}
...
inline void COGLES2Driver::createGLMatrix( float gl_matrix[16], const matrix4& m ) {
	memcpy( gl_matrix, m.pointer(), 16 * sizeof( f32 ) );
}


//! creates a opengltexturematrix from a D3D style texture matrix
inline void COGLES2Driver::createGLTextureMatrix( float *o, const matrix4& m ) {
	o[0] = m[0];
	o[1] = m[1];
	o[2] = 0.f;
	o[3] = 0.f;
...
ITexture* COGLES2Driver::createDeviceDependentTexture(IImage* surface, const path& name, void* mipmapData) {
...
bool COGLES2Driver::testGLError() {
	...
	case GL_INVALID_ENUM:
		Printer::log( "GL_INVALID_ENUM", ELL_ERROR );
		break;
	case GL_INVALID_VALUE:
		Printer::log( "GL_INVALID_VALUE", ELL_ERROR );
		break;
	case GL_INVALID_OPERATION:
		Printer::log( "GL_INVALID_OPERATION", ELL_ERROR );
		break;
	case GL_OUT_OF_MEMORY:
		Printer::log( "GL_OUT_OF_MEMORY", ELL_ERROR );
		break;
	};
	return true;
#else
...
bool COGLES2Driver::testEGLError() {
	...
	case EGL_NOT_INITIALIZED :
		Printer::log( "Not Initialized", ELL_ERROR );
		break;
	case EGL_BAD_ACCESS:
		Printer::log( "Bad Access", ELL_ERROR );
		break;
	case EGL_BAD_ALLOC:
		Printer::log( "Bad Alloc", ELL_ERROR );
		break;
	case EGL_BAD_ATTRIBUTE:
		Printer::log( "Bad Attribute", ELL_ERROR );
		break;
	case EGL_BAD_CONTEXT:
		Printer::log( "Bad Context", ELL_ERROR );
		break;
	case EGL_BAD_CONFIG:
		Printer::log( "Bad Config", ELL_ERROR );
		break;
	case EGL_BAD_CURRENT_SURFACE:
		Printer::log( "Bad Current Surface", ELL_ERROR );
		break;
	case EGL_BAD_DISPLAY:
		Printer::log( "Bad Display", ELL_ERROR );
		break;
	case EGL_BAD_SURFACE:
		Printer::log( "Bad Surface", ELL_ERROR );
		break;
	case EGL_BAD_MATCH:
		Printer::log( "Bad Match", ELL_ERROR );
		break;
	case EGL_BAD_PARAMETER:
		Printer::log( "Bad Parameter", ELL_ERROR );
		break;
	case EGL_BAD_NATIVE_PIXMAP:
		Printer::log( "Bad Native Pixmap", ELL_ERROR );
		break;
	case EGL_BAD_NATIVE_WINDOW:
		Printer::log( "Bad Native Window", ELL_ERROR );
		break;
	case EGL_CONTEXT_LOST:
		Printer::log( "Context Lost", ELL_ERROR );
		break;
	};
	return true;
#else
...
GLint COGLES2Driver::getTextureWrapMode(u8 clamp) const {
	...
	case ETC_MIRROR:
		...
		if ( FeatureAvailable[DREAM_OES_texture_mirrored_repeat] )
...
void COGLES2Driver::setWrapMode( const SMaterial& material ) {
	...
	for ( u32 u = 0; u < MaxTextureUnits; ++u ) {
		...
		if (queryFeature(EVDF_TEXTURE_NPOT) && !FeatureAvailable[DREAM_OES_texture_npot] &&
...
void COGLES2Driver::setBasicRenderStates( const SMaterial& material, const SMaterial& lastmaterial,
	...
	for ( u32 i = 0; i < MaxTextureUnits; ++i ) {
		...
		if ( FeatureAvailable[DREAM_EXT_texture_filter_anisotropic] )
			glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
			                 static_cast<GLfloat>( material.TextureLayer[i].AnisotropicFilter > 1 ? min_( MaxAnisotropy, material.TextureLayer[i].AnisotropicFilter ) : 1 ) );
#endif
	}
	testGLError();
...
//      if (FeatureAvailable[DREAM_ARB_multisample])
		{
...
void COGLES2Driver::setViewPort( const rectangle<s32>& area ) {
	rectangle<s32> vp = area;
	rectangle<s32> rendert( 0, 0, getCurrentRenderTargetSize().Width, getCurrentRenderTargetSize().Height );
...
void COGLES2Driver::drawStencilShadowVolume( const vector3df* triangles, s32 count, bool zfail ) {
	...
	glVertexAttribPointer( EVA_POSITION, 3, GL_FLOAT, false, sizeof( vector3df ), &triangles[0] );
	glStencilMask( ~0 );
	...
	if ( FeatureAvailable[DREAM_OES_stencil_wrap] ) {
...
void COGLES2Driver::drawStencilShadow( bool clearStencilBuffer, SColor leftUpEdge,
...
//! Draws a 3d line.
void COGLES2Driver::draw3DLine( const vector3df& start,
	...
	drawVertexPrimitiveList2d3d( vertices, 2, indices, 1, EVT_STANDARD, EPT_LINES );
}


//! Only used by the internal engine. Used to notify the driver that
//! the window was resized.
void COGLES2Driver::OnResize( const dimension2d<u32>& size ) {
...
bool COGLES2Driver::setVertexShaderPointer( const c8*, const void*, s32, bool, u16 ) {
	Printer::log( "Error: Please call services->setVertexPointer(), not VideoDriver->setVertexPointer()." );
	return false;
}
...
ITexture* COGLES2Driver::addRenderTargetTexture( const dimension2d<u32>& size,
        const path& name,
	...
	if ( queryFeature( EVDF_FRAMEBUFFER_OBJECT ) ) {
		...
		if ( rtt ) {
			...
			if ( tex ) {
				static_cast<COGLES2FBODepthTexture*>( tex )->attach( rtt );
				tex->releaseRef();
			}
			rtt->releaseRef();
		}
...
#endif
	{
		...
		if ( rtt )
			static_cast<COGLES2Texture*>( rtt )->setIsRenderTarget( true );
	}
...
bool COGLES2Driver::setRenderTarget( ITexture* texture, bool clearBackBuffer,
	...
	if ( texture ) {
		...
		CurrentRendertargetSize = dimension2d<u32>( 0, 0 );
	}
...
const dimension2d<u32>& COGLES2Driver::getCurrentRenderTargetSize() const {
...
IImage* COGLES2Driver::createScreenShot() {
	...
	if ( FeatureAvailable[DREAM_IMG_read_format] || FeatureAvailable[DREAM_OES_read_format] ) {
...
bool COGLES2Driver::setClipPlane( u32 index, const plane3df& plane, bool enable ) {
...
dimension2du COGLES2Driver::getMaxTextureSize() const {
