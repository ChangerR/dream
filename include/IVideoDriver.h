...
#ifndef __DREAM_I_VIDEO_DRIVER_H_INCLUDED__
#define __DREAM_I_VIDEO_DRIVER_H_INCLUDED__
...
enum E_TRANSFORMATION_STATE
{
	...
	//! Texture transformation
	ETS_TEXTURE_3,
#if _DREAM_MATERIAL_MAX_TEXTURES_>4
	//! Texture transformation
	ETS_TEXTURE_4,
#if _DREAM_MATERIAL_MAX_TEXTURES_>5
	//! Texture transformation
	ETS_TEXTURE_5,
#if _DREAM_MATERIAL_MAX_TEXTURES_>6
	//! Texture transformation
	ETS_TEXTURE_6,
#if _DREAM_MATERIAL_MAX_TEXTURES_>7
	//! Texture transformation
	ETS_TEXTURE_7,
#endif
#endif
...
irr::ISceneManager interface provides a lot of powerful classes
...
public:
	...
	MaxTextures (int) The maximum number of simultaneous textures supported by the driver. This can be less than the supported number of textures of the driver. Use _DREAM_MATERIAL_MAX_TEXTURES_ to adapt the number.
	...
	*/
	...
	information. */
	virtual ITexture* addTexture(const dimension2d<u32>& size,
	...
	virtual void makeNormalMapTexture(ITexture* texture, f32 amplitude=1.0f) const =0;
	...
	ITexture* target = driver->addRenderTargetTexture(dimension2d<u32>(128,128), "rtt1");
	...
	operations. */
	virtual void setViewPort(const rectangle<s32>& area) =0;

	//! Gets the area of the current viewport.
	/** \return Rectangle of the current viewport. */
	virtual const rectangle<s32>& getViewPort() const =0;
	...
	\param iType Index type, e.g. EIT_16BIT for 16bit indices. */
	...
	virtual void draw3DLine(const vector3df& start,
		const vector3df& end, SColor color = SColor(255,255,255,255)) =0;
	...
	virtual void draw3DBox(const aabbox3d<f32>& box,
		SColor color = SColor(255,255,255,255)) =0;
	...
	virtual void draw2DImage(const ITexture* texture,
		const position2d<s32>& destPos) =0;
	...
	virtual void draw2DImage(const ITexture* texture, const rectangle<s32>& destRect,
		const rectangle<s32>& sourceRect, const rectangle<s32>* clipRect =0,
		const SColor * const colors=0, bool useAlphaChannelOfTexture=false) =0;
	...
	virtual void draw2DPolygon(position2d<s32> center,
			f32 radius,
	...
	shadow node
	*/
	virtual void drawStencilShadowVolume(const array<vector3df>& triangles, bool zfail=true, u32 debugDataVisible=0) =0;
	...
	virtual const dimension2d<u32>& getScreenSize() const =0;
	...
	virtual const dimension2d<u32>& getCurrentRenderTargetSize() const =0;
	...
	virtual bool writeImageToFile(IImage* image, const path& filename, u32 param = 0) = 0;

	//! Writes the provided image to a file.
	/** Requires that there is a suitable image writer registered
	for writing the image.
	\param image Image to write.
	\param file  An already open IWriteFile object. The name
	will be used to determine the appropriate image writer to use.
	\param param Control parameter for the backend (e.g. compression
	level).
	\return True on successful write. */
	virtual bool writeImageToFile(IImage* image, IWriteFile* file, u32 param =0) =0;
	...
	virtual IImage* createImageFromData(ECOLOR_FORMAT format,
		const dimension2d<u32>& size, void *data,
	...
	If you no longer need the image, you should call IImage::drop().
	See IReferenceCounted::drop() for more information. */
	virtual IImage* createImage(ECOLOR_FORMAT format, const dimension2d<u32>& size) =0;
	...
	If you no longer need the image, you should call IImage::drop().
	See IReferenceCounted::drop() for more information. */
	_DREAM_DEPRECATED_ virtual IImage* createImage(ECOLOR_FORMAT format, IImage *imageToCopy) =0;
	...
	If you no longer need the image, you should call IImage::drop().
	See IReferenceCounted::drop() for more information. */
	_DREAM_DEPRECATED_ virtual IImage* createImage(IImage* imageToCopy,
			const position2d<s32>& pos,
			const dimension2d<u32>& size) =0;
	...
	virtual void OnResize(const dimension2d<u32>& size) =0;
	...
	serialization.
	\return The IAttributes container holding the material
	properties. */
	...
	*/
	...
	virtual bool setClipPlane(u32 index, const plane3df& plane, bool enable=false) =0;
	...
	*/
	...
	virtual stringc getVendorInfo() =0;
