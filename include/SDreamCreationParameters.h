...
struct SDreamCreationParameters
{
	//! Constructs a SDreamCreationParameters structure with default values.
	SDreamCreationParameters() :
		DeviceType(EIDT_BEST),
		DriverType(EDT_BURNINGSVIDEO),
		WindowSize(dimension2d<u32>(800, 600)),
		Bits(16),
		ZBufferBits(16),
		Fullscreen(false),
		Stencilbuffer(false),
		...
		SDK_version_do_not_use(DREAM_SDK_VERSION)
	{
	}

	SDreamCreationParameters(const SDreamCreationParameters& other) :
		SDK_version_do_not_use(DREAM_SDK_VERSION)
	{*this = other;}

	SDreamCreationParameters& operator=(const SDreamCreationParameters& other)
	{
